...
const util = {
  ...
  parseArgs: function (str, opts, cmd) {
    const reg = /"(.*?)"|'(.*?)'|`(.*?)`|([^\s"]+)/gi;
    let arr = [];
    let match;
    do {
  ...
  addArgsIndexes: function (cmd, opts) {
    var self = this
  ...
  getCmdArgsIndexes: function (cmd) {
  ...
  addTypeArgsIndexes: function (names, argsIdx) {
    var indexes = []
  ...
  addVariadicIndexes: function (cmd, opts, parts) {
    var self = this
  ...
  buildCommandArgs: function (passedArgs, cmd, execCommand, isCommandArgKeyPairNormalized) {
    ...
    const parsedArgs = this.parseArgs(passedArgs, types, cmd);
    ...
    if (!valid) {
      var required = cmd._args
        .filter(function (el) { return el.required })
        .map(function (e) { return e.name })
      var missing = required.slice(parsedArgs._.length)
      return '\n  Missing required argument(s) <' + missing.join('>, <') + '>. Showing Help:';
    }
